...
// Memory IDs for different maps (starting from 20 to avoid conflicts with alex_backend)
const ACTIVITIES_MEM_ID: MemoryId = MemoryId::new(20);
const USER_ACTIVITIES_MEM_ID: MemoryId = MemoryId::new(21);
const ARWEAVE_ACTIVITIES_MEM_ID: MemoryId = MemoryId::new(22);
const ACTIVITY_COUNTER_MEM_ID: MemoryId = MemoryId::new(23);
const USER_REACTIONS_MEM_ID: MemoryId = MemoryId::new(24);
const IMPRESSIONS_MEM_ID: MemoryId = MemoryId::new(25);
const VIEWS_MEM_ID: MemoryId = MemoryId::new(26);
...
// Viewers list: Vec<Option<Principal>> where Some = authenticated, None = anonymous
#[derive(CandidType, Serialize, Deserialize, Clone, Debug)]
pub struct ViewersList(pub Vec<Option<Principal>>);

#[derive(Debug, Clone)]
pub struct StorableViewersList(pub ViewersList);

impl Storable for StorableViewersList {
    fn to_bytes(&self) -> Cow<[u8]> {
        Cow::Owned(Encode!(&self.0).unwrap())
    }

    fn from_bytes(bytes: Cow<[u8]>) -> Self {
        Self(Decode!(bytes.as_ref(), ViewersList).unwrap())
    }
...
thread_local! {
    ...
    // Impressions: arweave_id -> count (simple counter, no dedup)
    pub static IMPRESSIONS: RefCell<StableBTreeMap<StorableString, u64, Memory>> = RefCell::new(
        StableBTreeMap::init(
            MEMORY_MANAGER.with(|m| m.borrow().get(IMPRESSIONS_MEM_ID)),
        )
    );

    // Views: arweave_id -> list of viewers (deduplicated for authenticated users)
    pub static VIEWS: RefCell<StableBTreeMap<StorableString, StorableViewersList, Memory>> = RefCell::new(
        StableBTreeMap::init(
            MEMORY_MANAGER.with(|m| m.borrow().get(VIEWS_MEM_ID)),
        )
    );
}
