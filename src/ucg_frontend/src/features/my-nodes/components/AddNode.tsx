		...
		onSubmit: async (values) => {
			if (authClient) {
				if (await authClient.isAuthenticated()) {
					dispatch(addNode({ actor, node: values }));
				} else {
				...
				<main className="container h-full w-full flex flex-col flex-grow justify-between gap-4">
					...
					{newNode ? (
						...
						>
							...
							<div className="flex flex-col items-start font-roboto-condensed font-medium text-black">
								<label className="text-lg" htmlFor="pub_key">
									Public Key
								</label>
								<input
									className={`w-full border border-gray-400 focus:border-gray-700 p-1 rounded text-xl ${
										formik.touched.pub_key &&
										formik.errors.pub_key
											? "border-red-500"
											: ""
									}`}
									id="pub_key"
									...
									onChange={formik.handleChange}
									onBlur={formik.handleBlur}
									value={formik.values.pub_key}
								/>
								{formik.touched.pub_key && formik.errors.pub_key && (
									<span className="text-red-400 text-sm">
										{formik.errors.pub_key}
									</span>
								)}
