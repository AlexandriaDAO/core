...
>("junoPortal/fetchBooks", async (from, { rejectWithValue, fulfillWithValue, getState }) => {
	try {
		// Get the current state
		const state = getState();

		// Access the limit from the portal state
		const limit = state.junoPortal.limit;
...
const junoPortalSlice = createSlice({
	name: "junoPortal",
	initialState,
	reducers: {
...
export const { setLimit, setView}  =  junoPortalSlice.actions;

export default junoPortalSlice.reducer;
