...
const Tabs = () => {
  const { UID, login, logout, ledgerService, actor } = useAuth();
  ...
  const fetchAccountInfo = async () => {
    if (UID) {
      const accountId = AccountIdentifier.fromPrincipal(actor.getPrincipal());
      setAccountId(ledgerService.shortAccountId(accountId));
  
      const balance = await actor.getBalance(accountId);
      setBalance(ledgerService.displayE8sAsIcp(balance));
    }
  };

  useEffect(() => {
    fetchAccountInfo();
    console.log("Account info: ", UID)
    console.log("LedgerService: ", accountId)
    ...
    <div className="flex justify-between items-center">
      ...
      <div className="authTab my-0">
        <div className="innerAuthTab">
          {UID && (
            <>
              <label className="border-right">{formatPrincipal(UID)}</label>
              <label className="border-right">{accountId}</label>
              <label className="border-right">{balance}</label>
            </>
          )}
          {UID ? (
            <label onClick={logout}>Logout</label>
