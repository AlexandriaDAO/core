...
const MessageProvider: React.FC<MessageProviderProps> = ({ children }) => {
  ...
  const [isLoading, setIsLoading] = useState<boolean>(false);
  const [error, setError] = useState<string | null>(null);
  const [randomAuthorId, setRandomAuthorId] = useState<string | null>(null);
  ...
  return (
    <MessageContext.Provider value={{ message, updateMessage, isLoading, error, randomAuthorId, setRandomAuthorId }}>
      {children}
    </MessageContext.Provider>
  );
};
