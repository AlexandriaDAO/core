...
# Determine the correct path to full_archive.mo
SCRIPT_DIR=$(dirname "$0")
if [ -f "$SCRIPT_DIR/full_archive.mo" ]; then
  ARCHIVE_FILE="$SCRIPT_DIR/full_archive.mo"
elif [ -f "$SCRIPT_DIR/../icrc7/full_archive.mo" ]; then
  ARCHIVE_FILE="$SCRIPT_DIR/../icrc7/full_archive.mo"
elif [ -f "full_archive.mo" ]; then
  ARCHIVE_FILE="full_archive.mo"
elif [ -f "src/icrc7/full_archive.mo" ]; then
  ARCHIVE_FILE="src/icrc7/full_archive.mo"
else
  echo "Error: Could not find full_archive.mo file"
  echo "Please make sure the file exists and you're running the script from the correct directory"
  rm -rf "$TEMP_DIR"
  exit 1
fi

echo "Starting NFT minting process from $ARCHIVE_FILE"
...
echo ""
echo "----------------------------------------"
echo "NFT Minting Process Finished for ICRC7"
echo "----------------------------------------"
...
echo "The canister now contains a 'dev_mint' function for local development."
