...
/// ```
#[query]
pub fn siws_delegation(session_id_hex: String) -> AuthResult<SignedDelegation> {
    ...
    let session = SOLSession::get(session_id)
    ...
    // Step 6: Create certified signature using callback pattern with double hash for IC authentication
    let certified_signature = DelegationTree::witness(session.witness_key, |witness| {
        DelegationTree::certify(certificate, witness)
    })?;

    // Step 9: Create signed delegation with certified signature
    let signed_delegation = SignedDelegation {
        delegation,
        signature: certified_signature,
    };

    // Step 10: Return the IC-certified signed delegation
