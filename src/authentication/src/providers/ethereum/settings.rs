/// Ethereum/MetaMask authentication settings with stable storage
/// Controls SIWE message format, session duration, and security parameters
...
use super::types::constants::{MAX_CHAIN_ID, MIN_CHAIN_ID};
use crate::core::storage::{get_memory, memory_ids, Memory};
use crate::core::time::{hours_to_nanos, minutes_to_nanos};

/// Ethereum authentication configuration
/// All SIWE message parameters and session controls
#[derive(CandidType, Deserialize, Clone, Debug)]
pub struct EthereumSettings {
    /// SIWE message validity in nanoseconds (default: 10 minutes)
    ...
    /// Domain for SIWE messages (e.g., "myapp.com")
    /// Must match the domain where authentication requests originate
    pub domain: String,

    /// URI for SIWE messages (e.g., "https://myapp.com")
    /// Used in SIWE message construction
    pub uri: String,
    ...
    /// Validates which Ethereum network signatures come from
    pub chain_id: u64,

    /// SIWE version (typically "1")
    /// Protocol version for SIWE message format
    pub version: String,
...
impl Default for EthereumSettings {
    fn default() -> Self {
        Self {
            ...
            salt: "ethereum-mulauth-salt".to_string(),
            chain_id: 1, // Ethereum mainnet
            version: "1".to_string(),
            statement: "Sign in to authenticate with your Ethereum wallet".to_string(),
        }
    }
}

impl Storable for EthereumSettings {
...
// Stable storage for Ethereum settings - survives canister upgrades
thread_local! {
    static ETH_SETTINGS: RefCell<StableCell<EthereumSettings, Memory>> = RefCell::new(
        StableCell::init(
            get_memory(memory_ids::ETHEREUM_SETTINGS),
            EthereumSettings::default()
        )
    );
}
...
impl EthereumSettings {
    /// Get current Ethereum settings (internal helper)
    ///
    /// This is a simple wrapper around the storage access.
    /// Used internally by other methods.
    pub fn get() -> Self {
        ETH_SETTINGS.with(|s| s.borrow().get().clone())
    }

    /// Update Ethereum settings with a closure pattern
    ///
    ...
    /// ```rust
    /// // Single field update
    /// let settings = EthereumSettings::update(|s| {
    ...
    ///     s.chain_id = 5; // Switch to Goerli testnet
    /// })?;
    /// ```
    fn update<F>(updater: F) -> Result<Self, String>
    where
        F: FnOnce(&mut Self), // F is a closure that takes &mut EthereumSettings
    {
        ETH_SETTINGS.with(|s| {
    ...
    fn validate(settings: &Self) -> Result<(), String> {
        ...
        if settings.chain_id < MIN_CHAIN_ID {
            return Err(format!(
                "Chain ID {} is below min chain id of {}",
                settings.chain_id, MIN_CHAIN_ID
            ));
        }

        if settings.chain_id > MAX_CHAIN_ID {
            return Err(format!(
                "Chain ID {} is above max chain id of {}",
                settings.chain_id, MAX_CHAIN_ID
            ));
        }
...
#[update]
pub fn update_ethereum_settings(new_settings: EthereumSettings) -> Result<String, String> {
    ...
    EthereumSettings::update(|settings| {
        *settings = new_settings;
    })?;

    Ok("Ethereum settings updated successfully".to_string())
}
...
/// Initialize Ethereum settings storage
/// Called during canister initialization
pub fn init_ethereum_settings() -> Result<(), String> {
    ...
    ETH_SETTINGS.with(|s| {
        ...
        if settings.domain.is_empty() {
            Err("Ethereum settings initialization failed".to_string())
        } else {
            Ok(())
        }
    })
