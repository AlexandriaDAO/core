...
import { clearWithdrawError, clearFeeError } from "../alexSlice";
...
const Withdraw: React.FC = () => {
	...
	const { unlocked: alexBalance, withdrawing, withdrawError, fee: alexFee, feeLoading, feeError } = useAppSelector((state) => state.balance.alex);
	...
	const availableBalance = alexBalance > 0 ? alexBalance.toFixed(4) : "0.0000";
	const currentFee = alexFee > 0 ? alexFee : 0.0001; // fallback fee
	const isValidAmount = Number(amount) > 0 && Number(amount) <= alexBalance && Number(amount) >= currentFee;
	...
	const handleMaxAlex = useCallback(() => {
		const userBal = Math.max(0, alexBalance - currentFee);
		setAmount(userBal.toFixed(4));
	}, [alexBalance, currentFee]);
	...
	const handleSubmit = useCallback(async (event: React.FormEvent) => {
		...
		if (!alexActor || !canWithdraw) return;
	...
	}, [alexActor, amount, destination, canWithdraw, dispatch]);
	...
	const handleOpenChange = (open: boolean) => {
		...
		if (open) {
			...
			if (alexFee <= 0 && alexActor) {
				dispatch(fee({ actor: alexActor }));
			}
		} else {
		...
		<Dialog open={isOpen} onOpenChange={handleOpenChange}>
			...
			<DialogContent className="w-auto max-w-md flex flex-col gap-6 font-roboto-condensed p-6" closeIcon={null} onOpenAutoFocus={(e) => e.preventDefault()}>
				<div className="text-center">
					<DialogTitle className="text-lg font-semibold">Withdraw ALEX</DialogTitle>
					<DialogDescription className="text-sm text-muted-foreground">
						Enter amount and destination to withdraw your ALEX
					</DialogDescription>
				</div>
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					<div className="bg-gray-200 dark:bg-gray-800 border border-gray-300 dark:border-gray-600 p-3 rounded-lg">
						<div className="flex justify-between items-center text-sm">
							...
							<span className="font-medium">
								{feeLoading ? "Loading..." : `${currentFee.toFixed(4)} ALEX`}
							</span>
						</div>
					</div>
					...
					{Number(amount) > 0 && Number(amount) < currentFee && (
						<Alert variant="danger" title="Amount Too Small">
							Please enter at least {currentFee.toFixed(4)} ALEX to cover network fees
						</Alert>
					)}

					{Number(amount) > alexBalance && (
						<Alert variant="danger" title="Insufficient Balance">
							Amount exceeds available ALEX balance
						</Alert>
					)}
				</form>
