...
export interface FetchMarketAudioParams {
    page?: number;
    pageSize?: number;
...
export interface MarketAudioResponse {
    audios: MarketAudio[];
    totalPages: number;
    totalCount: number;
    page: number;
...
export async function fetchMarketAudioNFTs({
    page = 1,
    pageSize = 8,
    currentUserPrincipal,
    signal
}: FetchMarketAudioParams = {}): Promise<MarketAudioResponse> {
    try {
        ...
        // Step 4: Filter for audio content types
        const audioNFTs: MarketAudio[] = [];
        ...
        console.log(`Found ${audioNFTs.length} audio NFTs out of ${filteredListings.length} filtered marketplace listings`);
        ...
        throw new Error("Failed to fetch market audio NFTs");
    }
}