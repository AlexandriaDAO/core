...
import { setPurchasing, fetchMarketBookNFTs } from "../marketSlice";
...
export const useBuyBook = () => {
    ...
    const buyMutation = useMutation({
        mutationFn: async ({ arweaveId, tokenId, price }: BuyBookParams) => {
            if (!user) {
        ...
        onSuccess: ({ arweaveId, price }) => {
            toast.success(`Book NFT purchased for ${price} ICP`);
            
            // Close the dialog
            dispatch(setPurchasing(""));

            // Refresh market data to update UI
            dispatch(fetchMarketBookNFTs({ page: 1, pageSize: 8, appendMode: false, currentUserPrincipal: user?.principal }));
        },
        onError: (error: Error) => {
            console.error("Error purchasing book NFT:", error);
            toast.error(`Failed to purchase book NFT: ${error.message}`);
        },
    });

    return {
        buyBook: buyMutation.mutate,
