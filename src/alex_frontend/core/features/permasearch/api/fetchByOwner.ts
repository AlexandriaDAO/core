...
export async function fetchByOwner({ query, order, after, tags, range, signal}: QueryParams): Promise<QueryResult> {
	try {
		const response = await fetch(ARWEAVE_GRAPHQL_ENDPOINT, {
			...
			body: JSON.stringify({
				query: `
					query Get($owners: [String!], $order: SortOrder, $after: String, $tags: [TagFilter!], $range: BlockFilter) {
						transactions(first: 100, owners: $owners, sort: $order, after: $after, tags: $tags, block: $range) {
							${TransactionFields}
						}
					}
				`,
				variables: { owners: [query] , order, after, tags, range },
			}),
			signal,
		});
