...
export default function ShelfDetail({ shelfId, userId }: ShelfDetailProps) {
			...
			<div className="flex flex-col gap-6 px-4 py-6 max-w-6xl mx-auto animate-fade">
				<Tooltip>
					<TooltipTrigger asChild>
						<Button variant="ghost" scale="sm" onClick={() => navigate({ to: "/app/valora" })} className="self-start h-9 w-9 p-0">
	...
	const handleBack = () => {
		if (userId) {
			navigate({ to: "/app/valora/user/$userId", params: { userId } });
		} else {
			navigate({ to: "/app/valora" });
		}
	};

	const handleOwnerClick = () => {
		navigate({ to: "/app/valora/user/$userId", params: { userId: shelf.owner } });
	};
							...
							<div className="flex flex-col gap-1.5">
								{shelf.appearsIn.map((id) => (
									<AppearsInShelf
										...
										onNavigate={() => {
											setOpen(false);
											navigate({ to: "/app/valora/shelf/$shelfId", params: { shelfId: id } });
										}}
									/>
								))}
							</div>
