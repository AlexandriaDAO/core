...
const InlineItemCreator: React.FC<InlineItemCreatorProps> = ({
	...
	const allShelves = useAppSelector(
		type === "Shelf" ? selectUserShelves : () => []
	);
	const currentShelf = useAppSelector(
		type === "Shelf" ? selectSelectedShelf : () => null
	);
	...
	const handleSubmit = useCallback(async () => {
		...
		if (type === "Nft") {
			console.warn(
				"handleSubmit called on Nft tab, which should not happen."
			);
		...
		try {
			...
			if (!finalContent) {
				toast.error(
					`Please ${type === "Markdown" ? "enter" : "select"} ${type.toLowerCase()} content`
				);
			...
			const errorMessage =
				error?.message ||
				"You can't add a shelf that has this shelf inside it.";
	...
	const handleSetShelfId = useCallback(
	...
	const contentTypeTabs = useMemo(
		() => (
			<div className="flex p-2 gap-2 border-b border-border font-serif">
				{(["Markdown", "Nft", "Shelf"] as ContentType[]).map(
					(contentType) => (
						<Button
							key={contentType}
							variant={
								type === contentType ? "secondary" : "ghost"
							}
					...
					<div className="flex-1 flex flex-col font-serif">
						<div className="p-4 flex-grow">
							...
							<p className="text-xs text-muted-foreground mt-1 font-serif">
								For larger content, consider uploading your
								markdown as an NFT.
							</p>
						</div>
						...
						>
							...
							{availableShelves.length > 0 ? (
								...
								<option value="" disabled>
									No available shelves to add
								</option>
							)}
						</select>
						{availableShelves.length === 0 && (
							<p className="text-sm text-amber-500 mt-2 font-serif">
								You don't have any other shelves that can be
								added. Create a new shelf instead.
							</p>
						)}
	...
	return (
		<div className="bg-background border border-border rounded-md shadow-sm mb-4 overflow-hidden font-serif">
			<div className="flex flex-col">
				...
				<div className="flex-1 min-h-[400px]">{renderActiveForm()}</div>
			</div>
		</div>
	);
};
