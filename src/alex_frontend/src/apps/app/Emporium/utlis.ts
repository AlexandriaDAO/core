...
export const transformLogEntry = ( log: LogEntry, user: string): TransformedLogEntry => {
	const action: LogAction = (() => {
		...
		if ("Sold" in log.action)
			return {
				type: "Sold",
				price: log.action.Sold.price,
				isBuyer: log.buyer.toString() === user ? true : false,
			};
		if ("ReimbursedToBuyer" in log.action)
			return { type: "ReimbursedToBuyer" };
		if ("Listed" in log.action)
			return { type: "Listed", price: log.action.Listed.price };
