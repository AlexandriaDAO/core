...
import { fetchNftTransactions, loadContentForTransactions } from "../transactions/transactionThunks";
import {
  createTokenAdapter,
  TokenType,
  TokenAdapter,
...
>(
  ...
  async (
    ...
    try {
      ...
      // ** Previously, we waited for all balances to be fetched before returning nftRecord.
      // ** Now, balances are fetched asynchronously without blocking, and nftRecord is returned immediately.

      nftEntries.forEach(async ([tokenId]) => {
      ...
      // Return the nftRecord immediately
      return nftRecord;
    } catch (error) {
      const errorMessage =
