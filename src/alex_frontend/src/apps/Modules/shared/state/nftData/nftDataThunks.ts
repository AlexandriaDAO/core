    ...
    try {
      ...
      const nftBalances = await Promise.all(
        nftEntries.map(async ([tokenId]) => {
          ...
          // we are dispatching the nft balance here and fetching the balance again NftDataFooter :/
          // dispatch(
          //   updateNftBalances({
          //     tokenId,
          ...
          //   })
          // );
          console.log("alex balance is  ", convertE8sToToken(alexBalance));
          return {
            tokenId,
            alexBalance: Number(convertE8sToToken(alexBalance)),
            lbryBalance: Number(convertE8sToToken(lbryBalance)),
          };
        })
      );
      nftBalances.sort((a, b) => b.alexBalance - a.alexBalance);
   

      // **Create a new sorted nftRecord**
      const sortedNftRecord = Object.fromEntries(
        nftBalances.map(({ tokenId }) => [tokenId])
      );

      const sortedOriginalNftRecord: Record<string, NFTData> = Object.fromEntries(
        nftBalances.map(({ tokenId }) => [tokenId, nftRecord[tokenId]]) // Align order
      );
      
      dispatch(setNfts(sortedOriginalNftRecord));


      const arweaveIds = nftBalances.map((id) => {
        return nftRecord[id.tokenId].arweaveId;
      });
      await dispatch(
        fetchNftTransactions(arweaveIds) as unknown as AnyAction
      ).unwrap();
      return sortedNftRecord;
    } catch (error) {
      const errorMessage =
