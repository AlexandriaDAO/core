...
import { Database, Globe, ArrowDown, BookOpen, Coins, AlertCircle, Loader2 } from "lucide-react";
...
const BibliothecaPage: React.FC = () => {
	const dispatch = useAppDispatch();
	const { books, loading, error, hasNext, loadMore, isEmpty } = useArweaveBooks();
	...
	const convertToBook = (arweaveBook: ArweaveBook): Book => {
		...
		if (!contentType) {
			const contentTypeTag = arweaveBook.tags?.find(tag => tag.name === "Content-Type");
		...
		return {
			...
			timestamp: new Date(arweaveBook.block.timestamp * 1000).toISOString()
		};
	};
						...
						<div className="pt-2 border-t">
							<p className="text-xs text-muted-foreground">
								{loading && books.length === 0 ? (
									...
									<>
										<span className="font-medium">{books.length}</span> files available
									</>
								)}
							</p>
						</div>
			...
			<div className="col-span-4 overflow-y-auto px-6">
				...
				{loading && books.length === 0 && (
					<div className="flex items-center justify-center py-8">
						...
						<span className="ml-2 text-muted-foreground">Loading book files from Arweave...</span>
					</div>
				)}
				...
				{isEmpty && !loading && (
					<div className="flex flex-col items-center justify-center py-8 text-center">
						...
						<p className="text-muted-foreground">No book files found</p>
						...
						{books.map((arweaveBook) => {
							...
							return (
								<BookCard 
									...
									actions={
										<>
											<AudioBook url={`https://arweave.net/${bookItem.id}`} />
