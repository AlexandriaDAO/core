...
const BibliothecaShelfPage: React.FC = () => {
	...
	const { books, loading, loadingMore, error, pagination, refreshShelfBookNFTs } = useShelfBookNFTs();
					...
					<div className="space-y-4">
						...
						<div className="pt-2 border-t">
							<p className="text-xs text-muted-foreground">
								<span className="font-medium">{books.length}</span> of <span className="font-medium">{pagination.totalCount}</span> listings shown
							</p>
						</div>
					</div>
				</div>
				...
				{loading ? (
					<div className="flex items-center justify-center py-12">
						<div className="text-center">
							...
							<p className="text-muted-foreground">Loading your listed book NFTs...</p>
						</div>
					</div>
				) : error ? (
					<div className="flex items-center justify-center py-12">
						<div className="text-center">
							<p className="text-destructive font-medium">Error loading listed book NFTs</p>
							<p className="text-muted-foreground text-sm mt-1">{error}</p>
							<Button 
								onClick={() => user?.principal && refreshShelfBookNFTs(user.principal, 1, 8, false)}
								className="mt-4"
								variant="outline"
							>
								Try Again
					...
					<div className="flex items-center justify-center py-12">
						<div className="text-center">
							...
							<p className="text-muted-foreground">You don't have any book NFTs listed for sale yet.</p>
						</div>
					</div>
				) : (
							...
							<BookCard 
								...
								actions={
									<>
										<EditButton 
											item={item} 
											currentPrice={item.price.replace(' ICP', '')}
