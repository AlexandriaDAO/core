...
const UploadBook = () => {
	...
	return (
		<Dialog open={open == AssetType.Book}>
			<DialogTrigger asChild>
				<div onClick={()=>dispatch(setOpen(AssetType.Book))} className="group cursor-pointer hover:shadow-lg transition-shadow duration-300 bg-gray-50 rounded-lg p-6 basis-60">
					<div className="flex flex-col items-center space-y-4">
						<div className="p-3 bg-purple-100 rounded-full group-hover:bg-purple-200 transition-colors">
							<Headphones size={24} className="text-purple-600" />
						</div>
						<h3 className="font-semibold text-lg">Book</h3>
						<p className="text-sm text-gray-600 text-center">Upload .epub files</p>
					</div>
				</div>
			</DialogTrigger>
			...
			<DialogContent closeIcon={null} className="sm:max-w-2xl flex flex-col gap-6 border-none focus:border-none focus-visible:border-none outline-none" onOpenAutoFocus={(e) => e.preventDefault()}>
				<header className="">
					<DialogTitle>Book Uploader</DialogTitle>
