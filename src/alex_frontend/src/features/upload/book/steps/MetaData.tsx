...
const MetaData = () => {
	...
	const { irys, metadata, cover, setMetadata, setCover} = useUploader()
	const { loading, metadata: metadataState } = useAppSelector(state=>state.upload);

	const formik = useFormik({
		initialValues: {
			title: metadataState?.title || "",
			creator: metadataState?.creator || "",
			fiction: metadataState?.fiction || false,
			language: metadataState?.language || "en",
			type: metadataState?.type || 0,
			categories: metadataState?.categories || [] as number[],
			era: metadataState?.era || 10,
		},
	...
	useEffect(() => {
        ...
        if (!cover) dispatch(previousScreen());
    }, [metadata, cover, dispatch]);
