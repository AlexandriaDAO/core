...
import { nextScreen, nextStatus, setOpen, setStatus } from "../../uploadSlice";
...
const Process = () => {
    ...
    const { status, percent, loading, error } = useAppSelector(state => state.upload);
    ...
    useEffect(() => {
        if (!user || !irys || !asset || !cover || !metadata) return;
	...
	const items:StepProps[] = [
		...
		},
		{
			...
			description: "Uploading your cover file to arweave",
		...
		},
		{
			...
			icon: status == 5 && !percent && <LoaderCircle size={26} className="text-info animate animate-spin"/>,
			status: status < 5 ? "wait" : status === 5 ? "process" : "finish",
		},
		{
			title: "Upload Success",
			description: "Your book has been uploaded",
			status: status < 6 ? "wait" : status === 6 ? "process" : "finish"
		}
	]

    return (
        <div className="flex flex-col gap-2">
            {error ? <Failed /> : status > 6 ? <Finished /> : <ProcessingSteps status={status} percent={percent} items={items} />}
