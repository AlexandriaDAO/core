...
import { setSelling, fetchUserAudioNFTs } from "../archiveSlice";
import { useAppSelector } from "@/store/hooks/useAppSelector";

const emporium_canister_id = process.env.CANISTER_ID_EMPORIUM!;

interface SellAudioParams {
    tokenId: string;
    price: string;
}

export const useSellAudio = () => {
    ...
    const sellMutation = useMutation({
        mutationFn: async ({ tokenId, price }: SellAudioParams) => {
            ...
            console.log('Selling audio NFT:', { tokenId, tokenIdNat: tokenIdNat.toString(), price, priceFormat: priceFormat.toString() });
        ...
        onSuccess: ({ tokenId, price }) => {
            toast.success(`Audio NFT listed for ${price} ICP`);
            ...
            if (user?.principal) {
                dispatch(fetchUserAudioNFTs({ userPrincipal: user.principal }));
            }
            queryClient.invalidateQueries({ queryKey: ["archive-audio-nfts"] });
        },
        onError: (error: Error) => {
            console.error("Error listing audio NFT:", error);
            toast.error(`Failed to list audio NFT: ${error.message}`);
        },
    });

    return {
        sellAudio: sellMutation.mutate,
