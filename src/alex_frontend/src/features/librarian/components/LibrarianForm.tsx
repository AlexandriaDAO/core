...
const LibrarianForm = () => {
	...
	const { actor, authClient } = useSession();

	const [librarianFormModal, setLibrarianFormModal] = useState(false);

	const handleCancel = () => {
		setLibrarianFormModal(false);
	};
		...
		onSubmit: async (values) => {
			if (authClient) {
				if (await authClient.isAuthenticated()) {
					dispatch(becomeLibrarian({ actor, librarian: values }));
					setLibrarianFormModal(false);
				} else {
	...
	return (
		<>
			<button onClick={() => setLibrarianFormModal(true)} className="px-2 py-1 bg-black rounded cursor-pointer text-[#F6F930] font-medium font-roboto-condensed text-base">
				Become Librarian
			</button>
			<Modal
				open={librarianFormModal}
