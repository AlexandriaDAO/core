...
export async function fetchById({ query, order, after, tags, range, signal}: QueryParams): Promise<QueryResult> {
	try {
		const response = await fetch(ARWEAVE_GRAPHQL_ENDPOINT, {
			...
			body: JSON.stringify({
				query: `
					query Get($ids: [ID!], $order: SortOrder, $after: String, $tags: [TagFilter!], $range: BlockFilter) {
						transactions(first: 100, ids: $ids, sort: $order, after: $after, tags: $tags, block: $range) {
							${TransactionFields}
						}
					}
				`,
				variables: { ids: [query] , order, after, tags, range },
			}),
			signal,
		});

		if (!response.ok) throw new Error(`HTTP ${response.status}: Failed to fetch transactions by ids`);
