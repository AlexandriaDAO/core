...
const TimeSort: React.FC = () => {
    ...
    const { sortByTime, found } = useAppSelector((state) => state.imporium.listings);

    return (
        <Button onClick={() => dispatch(toggleSortByTime())} variant="outline" scale="md" rounded="lg" className='font-syne border-border relative' disabled={Object.keys(found).length > 0}>
