...
// 	const alex_backend_canister_id = process.env.CANISTER_ID_ALEX_FRONTEND!;

// 	const tsk_seed = window.crypto.getRandomValues(new Uint8Array(32));
// 	const tsk = new vetkd.TransportSecretKey(tsk_seed);

// 	const ek_bytes_hex = await actorAlexLibrarian.encrypted_ibe_decryption_key_for_caller(tsk.public_key());

// 	const pk_bytes_hex = await actorAlexLibrarian.ibe_encryption_key();

// 	const k_bytes = tsk.decrypt(
// 		hexDecode(ek_bytes_hex),
// 		hexDecode(pk_bytes_hex),
// 		Principal.fromText(alex_backend_canister_id).toUint8Array()
// 	);
...
export const ibe_encrypt = async(actor: ActorSubclass<_SERVICE>, message:string)=> {
	...
	let ibe_principal = Principal.fromText(alex_wallet_canister_id);
