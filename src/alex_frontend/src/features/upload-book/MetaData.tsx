...
import { setIsMetaDataValid, updateMetadata } from "./uploadBookSlice";
import DDC from "@/data/categories";
import eras from "@/data/eras";
import Button from "../irys/components/Button";
...
const MetaData = () => {
	...
	const {metadata} = useAppSelector((state) => state.uploadBook);
	...
	// React.useEffect(() => {
	// 	dispatch(updateMetadata(formik.values));
	// }, [formik.values, dispatch]);


	// Add this effect to update the isValid state
	React.useEffect(() => {
		dispatch(updateMetadata({ ...formik.values}));
		dispatch(setIsMetaDataValid(formik.isValid));
	}, [formik.isValid, formik.values, dispatch]);
	...
	const handleSubcategoryChange = (subcategory: number) => {
        ...
        if (currentCategories.includes(subcategory)) {
            formik.setFieldValue(
                'categories',
                currentCategories.filter((sc) => sc !== subcategory)
            );
	...
	return (
		<section className="flex-grow h-full overflow-auto p-4 w-full flex flex-col text-sm gap-3 items-start justify-start">
