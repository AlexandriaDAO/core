...
>('balance/lbry/locked', async (_, { rejectWithValue, getState }) => {
	try {
		...
		// Get balance from LBRY canister
		...
		return rejectWithValue(error?.message || 'Failed to fetch LBRY spending balance');
	}
});
