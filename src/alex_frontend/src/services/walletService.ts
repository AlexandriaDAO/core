import { Keys } from "src/declarations/alex_backend/alex_backend.did";
...
export const saveKeys = async (publicKey: string, privateKey: string, slotIndex: number, UID: Principal): Promise<void> => {
  try {
    ...
    await alex_backend.save_keys(encryptedPublicKey, encryptedPrivateKey, slotIndex);
...
export const getKeys = async(): Promise<Keys[]> => {
  try {
    return await alex_backend.get_keys();
    } catch (error) {
...
export const getLibrarianKeys = async(principal: Principal): Promise<Keys[]> => {
  try {
    return await alex_backend.get_librarian_keys(principal);
  } catch (error) {
...
export const deleteKeys = async (slot: number): Promise<void> => {
  try {
    await alex_backend.delete_keys(slot);
