...
import arweaveReducer, {
	setSearchState,
} from "@/apps/Modules/shared/state/arweave/arweaveSlice";
...
import nftDataReducer, {
	setNFTs,
	clearNfts,
} from "@/apps/Modules/shared/state/nftData/nftDataSlice";
...
jest.mock("@/apps/Modules/LibModules/arweaveSearch/api/arweaveClient", () => {
	...
	return {
		fetchTransactions: jest
			.fn()
			.mockImplementation((nftIds, contentTypes, amount, after) => {
				...
				if (!after) {
					...
					const cursorIndex = allTransactions.findIndex(
						(tx) => tx.cursor === after
					);
					if (cursorIndex !== -1) {
				...
				for (
					let i = 0;
					i < count && currentIndex < allTransactions.length;
					i++
				) {
...
jest.mock(
	"@/apps/Modules/LibModules/contentDisplay/services/contentService",
	() => ({
		ContentService: {
		...
		fetchTokensForPrincipal: jest.fn().mockImplementation((params) => {
			return (dispatch: AppDispatch) => {
				...
				const start = params.page
					? (params.page - 1) * (params.itemsPerPage || 20)
					: 0;
...
jest.mock("@/apps/Modules/shared/components/TensorFlowPreloader", () => {
	...
	return {
		__esModule: true,
		default: function MockTensorFlowPreloader({
			onLoaded,
		}: {
			onLoaded: () => void;
		}) {
			...
			return mockReact.createElement(
				"div",
				{ "data-testid": "tensor-flow-preloader" },
		...
		SearchContainer: function MockSearchContainer({
			...
			return mockReact.createElement(
				...
				[
					...
					mockReact.createElement(
						"div",
						{ "data-testid": "data-source", key: "source" },
						dataSource
					),
					mockReact.createElement(
						"div",
						{ "data-testid": "filter-component", key: "filter" },
						filterComponent
					),
				]
			);
...
describe("Search Results Tests", () => {
	...
	describe("Permasearch Search", () => {
		it("should fetch and display all expected results", async () => {
			...
			await store.dispatch(
				performSearch({
					searchState: (store.getState() as RootState).arweave
						.searchState,
			...
			// Check content types
			const contentTypes = state.transactions.transactions.map(
				(tx: Transaction) =>
					tx.tags.find((tag) => tag.name === "Content-Type")?.value
			);

			const pngCount = contentTypes.filter(
				(type: string | undefined) => type === "image/png"
			).length;

			const jpegCount = contentTypes.filter(
				(type: string | undefined) => type === "image/jpeg"
			).length;
		...
		it("should handle pagination correctly", async () => {
			...
			expect(
				(store.getState() as RootState).transactions.transactions.length
			).toBe(10);

			// Get the last transaction for continuation
			const lastTransaction = (store.getState() as RootState).transactions
				.transactions[9];
			...
			const initialTransactions = [
				...(store.getState() as RootState).transactions.transactions,
			];

			// Create new transactions with timestamps greater than the last one
			const newTransactions = Array(10)
				.fill(0)
				.map((_, i) => ({
					id: `tx-new-${i}`,
					owner: `owner-new-${i}`,
					...
					block: {
						height: 200 + i,
						timestamp:
							lastTransaction.block!.timestamp + 1000 + i * 1000,
					},
					cursor: `cursor-new-${i}`,
			...
			if (
				!transactions[9] ||
				!transactions[9].block ||
				!transactions[10] ||
				!transactions[10].block
			) {
			...
			expect(secondBatchFirstTimestamp).toBeGreaterThanOrEqual(
				firstBatchLastTimestamp
			);
		});
	});
	...
	describe("Alexandrian Search", () => {
		...
		it("should handle pagination correctly", async () => {
			...
			// Verify initial results
			expect(
				Object.keys((store.getState() as RootState).nftData.nfts).length
			).toBe(20);
			...
			const secondBatchNfts = Array(20)
				.fill(0)
				.reduce(
					(acc, _, i) => {
						const index = i + 20;
