...
const BrowsePage: React.FC = () => {
	...
	return (
		<>
			<Helmet>
				<title>Browse Audio | Sonora | Alexandria</title>
				<meta name="description" content="Discover audio files from the Arweave permanent web. Browse, listen, and mint audio NFTs on Alexandria." />
			</Helmet>
				...
				<div className="w-full bg-card rounded-lg border p-5">
					<div className="space-y-4">
						...
						<p className="text-sm text-muted-foreground">
							Audio files from the permanent web, ready to
							discover and mint.
						</p>
				...
				{audios.map((arweaveAudio) => {
					...
					return (
						<AudioCard
							key={arweaveAudio.id}
							item={audioItem}
							actions={<MintButton item={audioItem} />}
						/>
					);
				})}
			</div>
