...
const BibliothecaLibraryPage: React.FC = () => {
	...
	const { books, loading, loadingMore, error, pagination, refreshBookNFTs } =
		useUserBookNFTs();
	...
	useEffect(() => {
		if (user?.principal && books.length === 0 && !loading && !loadingMore) {
			console.log(
				"[LibraryPage] Fetching user book NFTs for:",
				user.principal
			);
					...
					<div className="space-y-4">
						<div className="flex items-center justify-between">
							...
							<Button
								...
								onClick={() =>
									user?.principal &&
									refreshBookNFTs(user.principal, 1, false)
								}
								disabled={loading}
						...
						<p className="text-sm text-muted-foreground">
							Your personal book NFT collection and uploaded
							content.
						</p>
